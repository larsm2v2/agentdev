...
class HighPerformanceGmailOrganizer(GmailAIOrganizer):
    ...
    def _classify_emails_batch_single(self, emails: List[Dict]) -> List[Dict]:
        ...
        # Get response via helper that centralizes LLM access and stores last response
        try:
            response = self._fetch_and_hold_batch_response(batch_prompt)
    ...
    def _fetch_and_hold_batch_response(self, prompt: str) -> str:
        """Fetch a response for the given prompt from an available LLM and save it.

        Preference order:
        - instance attribute `llm` with `simple_chat`
        - server.multi_llm_manager.simple_chat if available
        - fallback to a safe, deterministic default JSON response
        The raw response is stored on `self._last_batch_response` for debugging.
        """
        # Try instance-level LLM first
        llm = getattr(self, "llm", None)
        if llm and hasattr(llm, "simple_chat"):
            resp = llm.simple_chat(prompt)
            self._last_batch_response = resp
            return resp

        # Try server-level multi-LLM manager
        server_mgr = getattr(getattr(self, "server", None), "multi_llm_manager", None)
        if server_mgr and hasattr(server_mgr, "simple_chat"):
            resp = server_mgr.simple_chat(prompt)
            self._last_batch_response = resp
            return resp

        # As a last resort, attempt to use legacy ai_chains if present (kept as fallback)
        legacy = globals().get("ai_chains")
        if legacy and hasattr(legacy, "simple_chat"):
            try:
                resp = legacy.simple_chat(prompt)
                self._last_batch_response = resp
                return resp
            except Exception:
        ...
        # Deterministic default: return a JSON array of default classifications
        default_resp = json.dumps([
            {
                "email_index": i + 1,
                "category": self._default_classification()["category"],
                "confidence": self._default_classification()["confidence"],
                "priority": self._default_classification()["priority"],
                "reasoning": "LLM unavailable - default"
            }
            for i in range(1)
        ])
        self._last_batch_response = default_resp
        return default_resp
