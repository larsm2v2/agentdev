...
1. Getting message IDs in a date range
2. Batching message IDs
3. Retrieving email batches with raw content and labels
4. Bulk assigning labels with AI
5. Applying labels to Gmail messages
"""
...
class EmailJobProcessor:
    ...
    def __init__(self, organizer_factory: OrganizerFactory):
        """Initialize with organizer factory.
    ...
    async def get_message_ids_in_range(self, start_timestamp: int, end_timestamp: int, max_ids: int = 10000) -> List[str]:
        ...
        Args:
            ...
            max_ids: Maximum number of IDs to retrieve
        ...
        # Create organizer and try service-backed paged listing
        ...
        if hasattr(organizer, 'service') and getattr(organizer, 'service', None):
            try:
                ids = await list_message_ids_in_range(
                    ...
                    max_ids=max_ids,
                    page_size=500,
                    label_ids=["INBOX"],  # Only include inbox messages
                )
                return ids
            except Exception as e:
                logger.warning(f"Service-backed list_message_ids_in_range failed: {e}")
        
        # Fallback: try organizer methods
        if hasattr(organizer, "search_messages"):
            ...
            results = organizer.list_message_ids_in_range(start_timestamp, end_timestamp)
            if asyncio.iscoroutine(results):
                results = await results
        else:
            return []
        ...
        if isinstance(results, list):
            ...
            # Check for messages list in dictionary
    ...
    def batch_message_ids(self, message_ids: List[str], chunk_size: int = 10) -> List[List[str]]:
        ...
        Returns:
            List of batches (lists of IDs)
        """
        # Normalize to strings and remove falsy values
        ids = [str(x) for x in message_ids if x]
        return list(chunk_list(ids, chunk_size))
    
    async def retrieve_email_batch(self, message_ids: List[str], include_raw_body: bool = True, max_body_length: int = 400) -> List[Dict]:
        """Retrieve email content for a batch of message IDs.
        
        Args:
            message_ids: List of message IDs to retrieve
            include_raw_body: Whether to include raw email body
        ...
        if not hasattr(organizer, "fetch_email"):
            # Return minimal placeholders if fetch not available
            return [{"id": mid, "message_id": mid} for mid in message_ids]
        
        # Create a semaphore to limit concurrency
        sem = asyncio.Semaphore(min(10, max(1, len(message_ids))))
        
        async def fetch_email(mid: str):
            """Fetch a single email and normalize the output."""
            async with sem:
                try:
                    # Call fetch_email (supports sync and async)
                    result = organizer.fetch_email(mid)
                    if asyncio.iscoroutine(result):
                        email_data = await result
                    else:
                        email_data = await asyncio.to_thread(lambda: result)
                    
                    # Initialize normalized email dict
                    normalized = {
                        "id": mid,
                        "message_id": mid,
                    }
                    
                    # Extract data based on common structures
                    if isinstance(email_data, dict):
                        data = email_data.get("data") or email_data
                        ...
                        if include_raw_body:
                            raw_body = data.get("raw_body") or data.get("raw") or data.get("body") or ""
                            if isinstance(raw_body, (bytes, bytearray)):
                                try:
                                    normalized["raw_body"] = raw_body.decode("utf-8", errors="replace")[:max_body_length]
                                except Exception:
                                    normalized["raw_body"] = str(raw_body)[:max_body_length]
                            else:
                                normalized["raw_body"] = str(raw_body or "")[:max_body_length]
                    else:
                        # Handle object-style email data
                        try:
                            normalized["subject"] = str(getattr(email_data, "subject", "") or "")
                            normalized["from"] = str(getattr(email_data, "from", "") or getattr(email_data, "sender", "") or "")
                            normalized["snippet"] = str(getattr(email_data, "snippet", "") or "")
                            labels = getattr(email_data, "labelIds", []) or getattr(email_data, "labels", [])
                            if isinstance(labels, list):
                                normalized["labels"] = ",".join(str(label) for label in labels)
                            else:
                                normalized["labels"] = str(labels)
                            
                            if include_raw_body:
                                raw_body = getattr(email_data, "raw_body", None) or getattr(email_data, "raw", None) or getattr(email_data, "body", None)
                                if isinstance(raw_body, (bytes, bytearray)):
                                    normalized["raw_body"] = raw_body.decode("utf-8", errors="replace")[:max_body_length]
                                else:
                                    normalized["raw_body"] = str(raw_body or "")[:max_body_length]
                        except Exception as e:
                            logger.warning(f"Error extracting email attributes: {e}")
                    
                    return normalized
                except Exception as e:
                    logger.error(f"Failed to fetch email {mid}: {e}")
                    return {"id": mid, "message_id": mid, "error": str(e)}
        
        # Fetch emails concurrently
        tasks = [fetch_email(mid) for mid in message_ids]
        results = await asyncio.gather(*tasks)
        
        return [r for r in results if r is not None]
    
    async def assign_labels_with_ai(self, emails: List[Dict], organizer=None):
        """Assign labels to emails using AI.
        ...
        # Skip if no AI capability
        if not hasattr(organizer, 'suggest_category'):
            logger.warning("Organizer doesn't support AI categorization")
            for email in emails:
                email["assigned_category"] = "Uncategorized"
            return emails
        
        # Process emails in parallel with semaphore to limit concurrent requests
        ...
        async def process_email(email):
            async with sem:
                try:
                    ...
                    body = email.get("raw_body", "") or email.get("snippet", "")
                    ...
                    email["assigned_category"] = "Uncategorized"
                    email["category_error"] = str(e)
                    return email
        
        tasks = [process_email(email) for email in emails]
        return await asyncio.gather(*tasks)
    
    async def apply_labels_to_emails(self, emails: List[Dict], organizer=None):
        """Apply assigned labels to emails in Gmail.
        
        Args:
            emails: List of email data with assigned_category
            organizer: Optional organizer (will create one if not provided)
...
class ShelvingJobProcessor(BaseJobProcessor):
    ...
    async def process(self, job_id: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"ShelvingJobProcessor: Starting shelving job {job_id}")
            await self.update_job_status(job_id, "running")
            
            # Get an organizer for Gmail operations
            organizer = self.organizer_factory.create_organizer("high_performance")
            
            # Get message IDs from parameters or retrieve them
            message_ids = parameters.get("message_ids", [])
            if not message_ids:
                # Extract message IDs based on date range
                query = ""
                ...
                service = getattr(organizer, 'service', None)
                if service:
                    message_ids = await list_message_ids_in_range(
                        service, 
                        query, 
                        max_ids=parameters.get("max_emails", 1000),
                        page_size=500,
                        label_ids=['INBOX']
                    )
            ...
            if self.job_manager:
                await self.job_manager.update_job(job_id, {"total_count": total_count})
            ...
            for batch in batches:
                # Fetch email content
                emails = await self.email_processor.retrieve_email_batch(batch)
                
                # Categorize emails
                categorized_emails = await self.email_processor.assign_labels_with_ai(emails)
                
                # Apply labels
                results = await self.email_processor.apply_labels_to_emails(categorized_emails, organizer)
                
                # Update counts
                processed_count += len(batch)
                shelved_count += sum(1 for r in results if r.get("success", False))
            ...
            results = {
                ...
                "job_type": "shelving",
                "status": "completed",
                "message": f"Successfully shelved {shelved_count} of {processed_count} emails"
            }
            
            # Mark as completed
            await self.update_job_status(job_id, "completed", results)
            logger.info(f"ShelvingJobProcessor: Successfully processed {shelved_count} of {processed_count} emails")
            return results
